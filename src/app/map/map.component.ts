  ...
  getcenterpoint() {
    this.mapservice.getcenterpoint().subscribe(date => {
      ...
      for (var ctp in this.ctpList) {
        ...
        if (this.ctpList[ctp].radius != null) {
          this.radius = Number(this.ctpList[ctp].radius);
          console.log('value ' + this.radius + '>>>>>' + this.ctpList[ctp].radius);
        } else {
          this.radius = 100;
          console.log('no value ' + this.radius);
        }
